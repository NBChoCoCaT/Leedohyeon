...
- **Theory:** Caballero & Engel (1999) (S,s) — primary; Sandmo (1971) — auxiliary 1; Blundell & Pistaferri (2003) — auxiliary 2. **(S,s) directional prediction (B-6 anchor):** Korean smallholder s_min ≈ 5,000만원 (tractor / combine 자가부담, 보수 추정) → T = 1.2M KRW SFFP / s_min ≈ **2.4%** → lump-sum **deeply within inaction region** → predicted β(op_cost) ≤ 0 (lump-sum absorbed by 변동비 / 소비, NOT triggering lumpy reinvestment). P2 results align: op_cost coefficients range from −4.18M (T1) to −0.49M (T3 MSE), Spec B mirrors (−5.03M to −0.27M); all consistent with inaction-region prediction. Auxiliary channels: Sandmo (precautionary labor → β(off_farm_income) ambiguous); Blundell-Pistaferri (consumption smoothing → β(consumption) ≥ 0 if liquidity-constrained). See `.claude/agents/domain-reviewer.md` Lens 1 B-6 + Lens 5 E-6/E-7 for the full anchor + AJAE/Food Policy desk-review / referee defense.
