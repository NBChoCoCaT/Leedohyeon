...
- **Theory:** Caballero & Engel (1999) (S,s) — primary behavioral; Kirwan (2009) *JPE* 117(1) / Ciaian et al. (2023) *Land Use Policy* 134 — primary incidence; Sandmo (1971) — auxiliary; Blundell & Pistaferri (2003) — auxiliary. **Two-margin theory test (P3b headline framing):** **(i) Incidence margin (CH4 rent capitalization)** — Korean per-farm flat-rate is a third design distinct from US per-hectare area-proportional (Kirwan 2009: ~25% rent capitalize) and EU per-hectare flat-rate (Ciaian et al. 2023: 46% short-run, 55% long-run capitalize). Per-farm structure breaks the area–subsidy coupling that drives capitalization in both alternatives → predict **β(rent_cost) ≈ 0**. **(ii) Behavioral margin ((S,s) inaction)** — s_min ≈ 5,000만원 (Korean tractor/combine 자가부담, 보수 추정), T = 1.2M KRW SFFP, **T/s_min ≈ 2.4%** → lump-sum **deeply within inaction region** → predict **β(op_cost_ex_rent) ≤ 0** (lump-sum absorbed by 변동비 / 소비, NOT triggering lumpy reinvestment). P2 results align with (ii): op_cost (raw, rent-inclusive) coefficients range −4.18M (T1) to −0.49M (T3 MSE) under Spec A; Spec B mirrors. P3b (Task #14) isolates rent_cost separately to test (i) and op_cost_ex_rent to confirm (ii) rent-net. **Joint two-margin pattern is publishable JAE/AJAE contribution regardless of single-margin significance**, because the framing reverses the 2018+ EU literature's "flat-rate amplifies capitalization" prior and offers Korea as a counter-example via the per-farm (vs per-hectare) design distinction. Auxiliary channels (CH1 Sandmo precautionary labor; CH2 Blundell-Pistaferri consumption smoothing) treated as robustness, not core. See `.claude/agents/domain-reviewer.md` Lens 1 B-6 + Lens 5 E-6/E-7 for the full anchor + AJAE/Food Policy desk-review / referee defense.
