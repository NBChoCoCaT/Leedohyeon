...
\bottomrule
\end{tabular}\\
\footnotesize\textit{Wild 클러스터 부트스트랩: manual Rademacher refit (B=9999, `fwildclusterboot` R 4.5.3 CRAN binary 미제공). Cluster: hh\_id. STATA boottest (11\_multiple\_testing.log)은 공통 hA\_min bandwidth 사용 (Step 5 rwolf2 fallback), per-outcome bandwidth와 달라 ±0.01 tolerance 직접 비교는 deferred. 보고된 p\_wild는 per-outcome MSE bandwidth에서의 신규 R-only inference. Spec별 4 outcome Holm stepdown. * p$<$0.10, ** p$<$0.05, *** p$<$0.01.}
\end{table}
