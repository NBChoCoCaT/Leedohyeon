\begin{table}[t]
\centering
\caption{Outlier Robustness Ladder --- Outcome: op\_cost}
...
\end{tabular}
\\
\footnotesize\textit{Cluster-robust SE (hh\_id) in parentheses. Bandwidths: T1=±500m\textsuperscript{2}, T2=±1000m\textsuperscript{2}, T3=±3300m\textsuperscript{2}. IHS uses asinh scale; Winsor on KRW. 24 IHS+w99 Spec A cells anchored to STATA 06\_robustness\_aux.log; Winsor p0.5/p99.5 (AJAE robustness, S2 per outlier-policy spec v1.1) is R-only.}
\end{table}


\begin{table}[t]
\centering
\caption{Outlier Robustness Ladder --- Outcome: off\_farm\_income}
...
\begin{table}[t]
\centering
\caption{Outlier Robustness Ladder --- Outcome: farm\_income}
