...
write_alpha3_table <- function(grid, path) {
  con <- file(path, "w")
  on.exit(close(con))
  ...
  cat("\\toprule\n", file = con)
  cat("Outcome & Bin & BW & Estimate & SE & p \\\\\n", file = con)
  cat("\\midrule\n", file = con)
  for (i in seq_len(nrow(grid))) {
    row <- grid[i, ]
    line <- sprintf("%s & %s & %s & %.2f & %.2f & %.3f \\\\\n",
                    row$outcome,
                    gsub("_", "\\\\_", row$own_bin),
                    row$bw_id,
...
write_alpha3_table(combined_grid, file.path(out_dir, "tab_alpha3_results_en.tex"))
log_msg("Phase 11: tab_alpha3_results_en.tex saved.")
