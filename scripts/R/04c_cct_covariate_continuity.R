...
for (cov in covariates) {
  ...
  cov_label <- gsub("_", "\\\\_", cov, fixed = TRUE)
  latex_rows <- c(latex_rows,
                  sprintf("\\multicolumn{4}{l}{\\textit{%s}} \\\\", cov_label))
...
tex_lines <- c(
  "\\begin{table}[t]",
  "\\centering",
  paste0("\\caption{Covariate continuity at the 0.5\\,ha cutoff ",
  ...
  "\\end{tabular}",
  "\\\\",
  paste0("\\footnotesize\\textit{Robust bias-corrected coefficient ",
...
writeLines(tex_lines, file.path(out_dir, "tab_cct_covariate_continuity_en.tex"))
...
message(sprintf("  Output: cct_covariate_results.rds + tab_cct_covariate_continuity_en.tex"))
