...
$\partial A_{own}/\partial T > 0$, the monotone-tenancy gradient
$\partial^2 A_{own} / \partial T\, \partial(1-s_0) > 0$, and the
capital-adjustment sub-prediction
$\partial \text{op\_cost\_ex\_rent} / \partial T \le 0$ if $T/\tau < 1$. The
...
\begin{equation}
\max_{C_1, C_2, a, L_{f,t}, L_{o,t}, K} \; U(C_1, L_{\ell,1}) + \beta\, \mathbb{E}_1\!\left[U(C_2, L_{\ell,2})\right]
...
\begin{multline}
\mathcal{L} \;=\; U(C_1, L_{\ell,1}) + \beta\, U(C_2, L_{\ell,2})
...
\paragraph{Comparative statics --- Step 1: $\partial W^*/\partial T < 0$.}
Differentiating (\ref{eq:appB-threshold}) implicitly with respect to $T_i$ and
using the wealth-bias property $\rho'(\cdot) > 0$:
\begin{equation}
\frac{\partial W_i^*}{\partial T_i} \;=\; -\, \frac{(R - \lambda_1/\lambda_2)\, p_{land}\, \rho'(W_i^* + T_i)}{(R - \lambda_1/\lambda_2)\, p_{land}\, \rho'(W_i^* + T_i) + \text{(other terms)}} \;<\; 0.
\label{eq:appB-step1}
\end{equation}
The threshold shifts down: SFFP-eligible households cross over to ownership at
lower baseline wealth.

\paragraph{Comparative statics --- Step 2: $\partial A_{own}/\partial T > 0$.}
...
\paragraph{Comparative statics --- Step 3: monotone tenancy gradient.}
Let $s_{0,i} = A_{own,i,2018}/A_{i,2018}$ be the baseline own-share. By
construction, households with low $s_{0,i}$ (predominantly tenants) sit closer
to the indifference threshold $W^*$ in baseline wealth: they have not
previously crossed and have higher $\varphi(W^*)$ at the threshold. Households
with $s_{0,i} = 1$ (pure owner-operators) are inframarginal---their wealth
exceeds $W^*$ by a finite margin, so $\varphi(W_i^*) \to 0$ for them.
Differentiating (\ref{eq:appB-step2}) with respect to $(1 - s_{0,i})$ and using
$\partial \varphi(W^*) / \partial s_0 \le 0$:
\begin{equation}
\boxed{\frac{\partial^2 A_{own,i}}{\partial T_{SFFP}\, \partial(1 - s_{0,i})} \;>\; 0,}
\label{eq:appB-step3}
\end{equation}
recovering main-text equation~\ref{eq:CO-2}. This is the empirical signature
uniquely diagnostic of wealth-bias.
...
\paragraph{Sufficient conditions.} The derivations above require:
\begin{description}
\item[(SC1)] $\rho'(W) > 0$ bounded away from zero on the eligibility window
  $W \in [W^* - \epsilon, W^* + \epsilon]$ for some $\epsilon > 0$. Ensures
  wealth-bias is not asymptotically flat.
\item[(SC2)] $W^*(T)$ is unique --- holds under strict quasi-concavity of the
  own-versus-rent indifference curve, satisfied if $F$ is strictly concave in
  land and $\rho$ is strictly increasing.
\item[(SC3)] Participation slack at the threshold-crossing margin:
  $V_{own}(W^*, T) > V_{outside}$, where $V_{outside}$ is the value of pure
  off-farm employment. Empirically supported by FHES Wave~1 take-up rates of
  92.3\% among eligible households.
\item[(SC4)] $T_{SFFP} < \tau$ for the capital sub-prediction (Step 4).
  Verified empirically by (\ref{eq:appB-ratio}).
\item[(SC5)] Panel stationarity of $W_i$ relative to $T_i$: the running
  variable $rv_{2018}$ measured at baseline pre-policy ensures
  Cov($W_{i,2018}, T_{i,2020+}$) = 0 by construction, satisfying the standard
  DiD-RD identification condition.
\end{description}
...
predicts $\partial A_{own,i}/\partial T > 0$ for each treated household but
...
of (\ref{eq:appB-EK-foc-C})--(\ref{eq:appB-EK-foc-h}) with respect to $T$
yields, after lengthy but standard manipulations,
\begin{equation}
\frac{\partial (\mu/\lambda)}{\partial T} \;=\; \frac{\partial w_{f,i}}{\partial T} \;=\; f(U_{CC}, U_{Cl}, F_{LL}, m) \;\ne\; 0
\label{eq:appB-EK-step1}
\end{equation}
in general, with sign depending on the curvature of $U$ in $(C, \ell_l)$.

\paragraph{Comparative statics --- Step 2: $\partial \ell_o / \partial T$.}
...
text (Table~\ref{tab:alpha3-predictions}) with three additional columns:
empirical magnitudes from the P3b-2 analysis (where available), the
falsification trigger that would reject each channel, and the journal-grade
implication of falsification.

\begin{table}[ht]
\centering
\caption{Expanded reduced-form-to-econometric-$\beta$ mapping with empirical fit and falsification triggers.}
\label{tab:appB-mapping}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{1.7cm} p{1.9cm} p{1.9cm} p{0.9cm} p{2.6cm} p{2.4cm} p{1.6cm}}
\toprule
Channel & Outcome & Theory sign & Econ. $\hat\beta$ & Empirical fit (P3b-2) & Falsification trigger & Reframe \\
\midrule
Liquidity primary \#1 \citep{CarterOlinto2003_liquidity} & $A_{own}$ & $>0$ & $\beta_1$ & T2: $+1{,}089$ m\textsuperscript{2} pure tenant ($p\!=\!.033$); T3: $+408$ ($p\!=\!.003$) & $\hat\beta_1 = 0$ uniformly & Null estimate \\
Liquidity gradient \citep{CarterOlinto2003_liquidity} & $A_{own} \times (1-s_0)$ & $> 0$, mono in $1-s_0$ & $\beta_2$ & T2 monotone: $-1738$, $-600$, $-393$, $0$ across quintiles & $\hat\beta_2 = 0$ OR $s_0\!=\!1$ same as $s_0\!=\!0$ & Universal income effect (drop wealth-bias) \\
Capital adj. \citep{Kazukauskas2013_disinvestment} & op\_cost\_ex\_rent & $\le 0$ if $T/\tau \!<\! 1$ & $\beta_3$ & T1: $-4.02$M ($p\!=\!.055$); T2: $-3.22$M ($p\!=\!.079$) & $\hat\beta_3 > 0$ across bandwidths & Reconsider $\tau$ calibration \\
Labor (aux.) \citep{EswaranKotwal1986_supervision} & off\_farm\_income & $\ne 0$ under non-sep. & $\beta_4$ & TBD (auxiliary in P3b-2) & $\hat\beta_4 = 0$ all bandwidths & Drop supervision channel; primary survives \\
\midrule
\multicolumn{7}{l}{\textit{Ex-theory (B.1) --- aggregate-equilibrium implication:}} \\
Incidence \citep{Floyd1965_incidence, AlstonJames2002_incidence} & unit\_rent & $< 0$ via incidence closure & $\beta_5$ & T1: $-48$ to $-130$ across non-pure-owner bins; T2 pass-through $-11.1\%$ & N/A (not a theory test) & N/A \\
\bottomrule
\end{tabular}
\end{table}
...
supported; rejection of either row reframes the paper as indicated. The
``Capital adj.'' row is the secondary primary outcome; its rejection (positive
$\hat\beta_3$) would not falsify the primary mechanism but would force a
reconsideration of the calibrated adjustment threshold $\tau$. The ``Labor''
row is auxiliary; rejection of F2 narrows the contribution to a single
non-separability margin. The ``Incidence'' row is reported as evidence
consistent with the model's empirical footprint but is not a primary
theoretical contribution (see main text \S 3.4 equilibrium rent caveat).

\paragraph{Joint test --- F1 + F2.} If both F1 and F2 trigger, the AHM
separability null (\ref{eq:null}) cannot be rejected. The paper repositions as
a precise developed-country null estimate, complementing
\citet{LaFaveThomas2016_markets}.
