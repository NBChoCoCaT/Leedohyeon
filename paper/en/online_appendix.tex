...
uniquely diagnostic of wealth-bias.\footnote{\textbf{Partition-rule
...
\item[(SC2.5) --- \textsc{sufficient-for-sign} (Step 3 monotone gradient).]
  Baseline wealth $W_i$ and baseline own-share $s_{0,i}$ are negatively
  correlated within the eligibility window:
  $\text{Cov}(W_i, s_{0,i} \mid rv_{2018} \in [-h, +h]) < 0$ for the working
  bandwidth $h$. This is the load-bearing primitive behind
  $\partial \varphi(W^*)/\partial s_{0,i} \le 0$ (used at L196 in Step 3); pure
  tenants ($s_{0,i} = 0$) face the largest threshold-crossing density while
  pure owner-operators ($s_{0,i} = 1$) sit inframarginally. Empirically
  supported by FHES Wave 1: $\widehat{\text{corr}}(W_i, s_{0,i})$ within
  $\pm 1{,}000$~m\textsuperscript{2} of cutoff is negative (placeholder
  estimate; finalized in \S\ref{sec:results} post-P3 run).
  % FILL post-§5 P3: report \widehat{corr}(W_i, s_{0,i}) within h ∈ {500, 1000} m².
...
\midrule
Liquidity primary \#1 \citep{CarterOlinto2003_liquidity} & $A_{own}$ & $>0$ & $\beta_1$ & T1: TBD; T2: $+1{,}089$ m\textsuperscript{2} pure tenant ($p\!=\!.033$); T3: $+408$ ($p\!=\!.003$) & $\hat\beta_1 \le 0$ at T2 \emph{and} T3 & Reject H$_1$ if $p(\hat\beta_1 > 0) > .10$ at both T2 and T3 & Null estimate \\
Liquidity gradient \citep{CarterOlinto2003_liquidity} & $A_{own} \times (1-s_{0,i})$ & $> 0$, mono in $1-s_{0,i}$ & $\beta_2$ & T1: TBD; T2 monotone across bins: $\{$ pure\_tenant $-1738$; low\_owner $-600$; mixed $-393$; pure\_owner $0$ ref $\}$; T3: TBD & F1 fires if $\hat\beta_2 \le 0$ at T2 \emph{or} four-bin point estimates fail monotone-in-$(1-s_{0,i})$ ordering at T1 \emph{and} T2 & F1 fires if $p(\hat\beta_2 > 0) > .10$ at T2 \emph{and} four-bin point estimates not weakly monotone-decreasing in $s_{0,i}$ at T1 \emph{or} T2 (Holm-corrected) & Universal income effect (drop wealth-bias) \\
Capital adj. (primary \#2) \citep{Kazukauskas2013_disinvestment} & op\_cost\_ex\_rent & $\le 0$ if $T_{SFFP}/\tau \!<\! 1$ & $\beta_3$ & T1: $-4.02$M~KRW ($p\!=\!.055$); T2: $-3.22$M~KRW ($p\!=\!.079$); T3: TBD & $\hat\beta_3 > 0$ at \emph{any} bandwidth & Reconsider $\tau$ if $\hat\beta_3 > 0$ at T1 or T2 ($p < .10$); robustness range $\tau \in [20\text{M}, 40\text{M}]$~KRW & Reconsider $\tau$ calibration (\S\ref{sec:calibration}) \\
Labor (auxiliary) \citep{EswaranKotwal1986_supervision} & off\_farm\_income & $\ne 0$ under non-sep. & $\beta_4$ & T1: TBD; T2: TBD; T3: TBD (queued for next cycle) & F2: $\hat\beta_4 = 0$ all bandwidths & F2 fires if $p(\hat\beta_4 \ne 0) > .10$ at all three bandwidths; informative, not rejecting (EK-1 sign indeterminacy per main-text \S\ref{sec:predictions}) & Drop supervision channel; primary survives \\
\midrule
\multicolumn{8}{l}{\textit{Ex-theory (B.1) --- aggregate-equilibrium implication, not a theory test:}} \\
Incidence (ex-theory) \citep{Floyd1965_incidence, AlstonJames2002_incidence} & unit\_rent\_price & $< 0$ via incidence closure & $\beta_5$ & T1: $-48$ to $-130$ KRW/m\textsuperscript{2} across non-pure-owner bins; T2 pass-through $-11.1\%$; T3: TBD & N/A (not a theory test) & N/A & N/A \\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}
