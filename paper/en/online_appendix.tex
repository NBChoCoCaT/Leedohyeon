...
\item[(SC2.5) --- \textsc{sufficient-for-sign} (Step 3 four-bin monotone gradient).]
  ...
  wealth has already exceeded $W^*$ (no rent-to-own crossing remains).
  \textbf{Empirical support (FHES Wave 1, 2018 baseline, $|rv_{2018}| \le 1{,}000$~m\textsuperscript{2}, wealth proxy $W_i = \text{farm\_income}_{2018} - \text{debt\_total}_{2018}$, threshold $W^* = 5{,}335{,}757$~KRW = pure-tenant median wealth within bandwidth):} cross-tabulated sub-threshold-mass shares are $\{0.73, 0.69, 0.83, 0.82, 0.82\}$ across the five bins $\{$pure tenant, low\_owner, mixed, high\_owner, pure\_owner$\}$. Pure tenants exhibit the highest crossing density as predicted; the empirical pattern is monotone-decreasing from pure\_tenant to low\_owner (.73 → .69) but the interior bins (mixed, high\_owner, pure\_owner) cluster at .82, so the SC2.5 monotonicity condition holds only directionally (pure\_tenant $>$ interior $\approx$ pure\_owner) rather than strictly bin-wise. The 5-bin SC2.5 inequality reads: $\Pr_{\text{pure tenant}} > \Pr_{\text{low\_owner}} \approx \Pr_{\text{mixed}} \approx \Pr_{\text{high\_owner}} \approx \Pr_{\text{pure\_owner}}$. The F1 falsification test in \S\ref{sec:predictions} is operationalized via 5-bin point-estimate monotonicity on $\hat\beta_1$, which is robust to this weakened SC2.5 condition.\footnote{The four-bin sub-threshold-mass
...
\midrule
Liquidity primary \#1 \citep{CarterOlinto2003_liquidity} & $A_{own}$ & $>0$ & $\hat\beta_1$ & T1: $+1{,}492$ pure tenant ($p\!=\!.051$); T2: $+1{,}089$ ($p\!=\!.033$); T3 ($h\!=\!3{,}300$): $+338$ ($p\!=\!.013$) & $\hat\beta_1 \le 0$ at T2 \emph{and} T3 & Reject H$_1$ if $p(\hat\beta_1 > 0) > .10$ at both T2 and T3 & Null estimate \\
Liquidity gradient \citep{CarterOlinto2003_liquidity} & $A_{own} \times (1-s_{0,i})$ & $> 0$, mono in $1-s_{0,i}$ & $\hat\beta_2$ & T1: $\{1{,}492, 755, 212, -328, 0\}$ across bins (pure\_tenant $p\!=\!.051$, low\_owner $p\!=\!.011$); T2: $\{1{,}089, 410, 393, -101, 0\}$ (pure\_tenant $p\!=\!.033$, low\_owner $p\!=\!.051$, mixed $p\!=\!.063$); T3 ($h\!=\!3{,}300$): $\{338, 107, 416, -44, 0\}$ & F1 fires if $\hat\beta_2 \le 0$ at T2 \emph{and} four-bin point estimates fail weakly monotone in $(1-s_{0,i})$ at T2 (strict-AND boolean) & F1 fires if $p(\hat\beta_2 > 0) > .10$ at T2 \emph{and} four-bin point estimates not weakly monotone in $(1-s_{0,i})$ at T2 (Holm-corrected across the two conditions) & Universal income effect (drop wealth-bias) \\
Capital adj. (primary \#2) \citep{Kazukauskas2013_disinvestment} & op\_cost (pooled) & $\le 0$ if $T_{SFFP}/\tau \!<\! 1$ & $\hat\beta_3$ & T1: $-4.18$M~KRW ($p\!=\!.047$); T2: $-3.35$M~KRW ($p\!=\!.069$); T3 ($h\!=\!1{,}483$): $-0.49$M~KRW ($p\!=\!.79$) & $\hat\beta_3 > 0$ at \emph{any} bandwidth & Reconsider $\tau$ if $\hat\beta_3 > 0$ at T1 or T2 ($p < .10$); robustness range $\tau \in [20\text{M}, 40\text{M}]$~KRW & Reconsider $\tau$ calibration (\S\ref{sec:calibration}) \\
Labor (auxiliary) \citep{EswaranKotwal1986_supervision} & off\_farm\_income & $\ne 0$ under non-sep. & $\hat\beta_4$ & Estimated 5-bin $\times$ 3-bw in \S~5 (exploratory; methodological caveat that the cell-level rdrobust differs from the pooled DiD-RD spec; pooled $\hat\beta_4$ statistically near-zero) & F2: $\hat\beta_4 = 0$ all bandwidths & F2 fires if $p(\hat\beta_4 \ne 0) > .10$ at all three bandwidths; informative, not rejecting (EK-1 sign indeterminacy per main-text \S\ref{sec:predictions}) & Drop supervision channel; primary survives \\
\midrule
