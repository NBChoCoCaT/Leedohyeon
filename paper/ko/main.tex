...
본 연구는 통계청 농가경제조사(FHES) 1차 패널, 2018--2022년을 활용한다. 전체 패널은 $3{,}614$개 농가에 걸친 $14{,}474$개 농가-연도로 구성된다. 헤드라인 DiD-RD 추정을 위해 \emph{대칭 관측가능 자격 스크리닝}을 적용한다: 처치 농가와 통제 농가 모두 컷오프 자체를 제외한 3개 FHES-관측가능 SFFP 자격 기준, 즉 (ii) 농가-합산 자작 농지 $< 1.55$\,ha와 (vi) 농가 농외소득 $< 4{,}500$만\,KRW을 충족하도록 요구된다. 어느 기준이라도 충족하지 못하는 농가는 컷오프 \emph{양쪽}에서 제외된다. 결과 분석 표본은 $2{,}776$개 농가에 걸친 $N = 11{,}010$ 농가-연도이며 처치집단 평균 $\bar D = 0.407$이다.\footnote{FHES 패널은 통계청에서 운영하며 통계데이터통합서비스(MDIS, \url{https://mdis.kostat.go.kr/})를 통해 제한 마이크로데이터 조건으로 접근 가능하다. 전체 패널 통계($14{,}474$ 농가-연도, $3{,}614$ 농가)와 대칭 스크리닝 분석 통계($11{,}010$ 농가-연도, $2{,}776$ 농가, $\bar D = 0.407$)는 저자가 \texttt{panel\_2018\_2022.dta}에 대해 내부 검증한 결과이며(로그 일자 2026-05-06), 대칭 구성 스크립트는 \texttt{scripts/R/01d\_symmetric\_clean.R}(로그 일자 2026-05-20)이다; 둘 다 재현 패키지에 미러된다. 심사자와 재현자는 MDIS 신청을 통해 동등한 패널 구성을 요청할 수 있으며; 합성-생성기 경로(\texttt{scripts/R/synthetic/}, AEA DCAS v1.0 코드 전용 검증)는 제한 마이크로데이터를 공개하지 않고도 식별-관련 공변량 적률과 컷오프 밀도를 보존한다.}
...
\input{../../scripts/R/_outputs_symmetric/tab_descriptives_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_het_own_share_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_main_did_rd_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_ch4_rent_decomposition_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_cjm_density_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_cct_covariate_continuity_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_fuzzy_bin_sensitivity_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_rob_outlier_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_wild_bootstrap_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_dropped_balance_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_attrition_placebo_full_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_placebo_cutoffs_ko.tex}
...
\input{../../scripts/R/_outputs_symmetric/tab_sido_robustness_ko.tex}
